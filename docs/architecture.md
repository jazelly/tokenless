...
An explicitly consented managed-profile import may copy opaque cookie rows for selected ChatGPT, Claude, or Grok domains into the local managed profile. It excludes Gemini/Google, broad X/Twitter sessions, and shared browser storage; it reports only provider-level row counts and never logs, returns, or transmits cookie contents. Visible adapters do not export browser state. Direct adapters use provider-owned clients or documented public APIs with environment-only credentials and never import subscription browser credentials into the broker. The broker is not an arbitrary-path proxy. Prompt construction redacts obvious secrets and includes only user-approved files and context.
