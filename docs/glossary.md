...
|---|---|---|
| **Tokenless API** | The provider-access product layer of Tokenless: its daemon, CLI, configuration, control surfaces, and Universal API. It owns provider-turn routing and lifecycle and exposes the provider-facing HTTP/API boundary; it is separate from the Tokenless Harness. | Tokenless Harness, provider's official API |
...
| **Direct Mode** | The public V1 API route selected by `tokenless.execution_mode: direct`. Tokenless completes the provider turn through a direct provider protocol and the selected direct provider implementation. | Official provider API, visible browser automation |
...
| **Auth Context** | A provider-scoped set of explicitly selected session inputs and a declared lifetime used by one direct provider implementation. | Browser profile, account |
...
- **Direct Mode** is the API selector. When discussing implementation, name the concrete direct provider implementation when that distinction matters.
