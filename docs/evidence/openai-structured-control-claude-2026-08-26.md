...
Revision `ef7dbbe` then classified that short failure without retaining provider text. The real page was authenticated, signed in, visible, and focused; its composer was editable, active, and held the browser selection. Exactly one Send control was visible and disabled, with no enabled alternative, visible dialog, transient announcement, or structured session blocker. The next bounded observer hovers only that disabled control and classifies any tooltip or accessible description as rate, plan, model, attachment-processing, unavailable, unknown, or absent without retaining the underlying text.
