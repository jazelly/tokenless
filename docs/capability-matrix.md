...
The opt-in local [API proxy](api-proxy-integration.md) covers `conversation.chat` only, for any provider enabled on the resolved profile, named as `tokenless/<provider>`. Specialized comparison, Search, Image, Code, Agent, and Video outcomes stay on the task API so their structured output is preserved.
