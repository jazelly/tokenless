...
| Qwen | Unavailable | Generic Markdown upload passed, but full Harness jobs intermittently showed no card and submit produced no visible transition |
...
Generic `file.upload` remains advertised for Claude, Grok, Qwen, Perplexity, and Meta AI because their Markdown upload evidence is independent of Harness protocol compliance. Private provider-turn V0, Harness Auto, and the semantic router require `harness-attachment-roundtrip` evidence, so these generic-only routes cannot enter Harness candidate selection. Arena has no Markdown `file.upload` route.
...
Browser image requests may carry one 8 MiB-or-smaller PNG, JPEG, or WebP `reference_image` data URL. Such requests require `image.edit`, `image.input`, `file.upload`, and `artifact.download`; remote URLs and direct-mode reference images fail before provider submission. No provider currently satisfies that complete Browser requirement set after Arena's generic `file.upload` route was removed. Its historical image-edit evidence remains valid for that image-only surface but cannot establish Harness Markdown support.
...
Qwen's visible **Select Mode** → **Upload attachment** path is implemented and generic Markdown `file.upload` is experimental. Small Markdown, a large benign document, the exact compiled bytes, and the Harness filename pattern were each accepted in focused runs. Full Harness jobs remained intermittent: some produced no visible card within 120 seconds, while one accepted the card but submit produced no visible transition. This evidence advertises generic upload but not Harness eligibility.
...
Dola is registered as an experimental signed-in provider. Its general `conversation.chat` and Markdown `file.upload` routes are Harness-verified, while `image.generation` and `artifact.download` remain independently experimental on the Create Image surface.
