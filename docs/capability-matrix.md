...
The local [OpenAI-compatible Arena adapter](openai-compatible-api.md) exposes only `arena:max` for ordinary non-streaming text chat. Each request starts a new Direct conversation; bounded multi-message text history uses stateless transcript replay. Specialized comparison, Search, Image, Code, Agent, and Video results stay on the task API so their structured output is preserved.
