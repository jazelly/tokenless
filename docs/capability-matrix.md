...
The authenticated `POST /v1/images/generations` endpoint is the canonical image input for browser and direct execution. Browser auto routing intersects enabled, usable providers with `conversation.chat`, `image.generation`, and `artifact.download`; Gemini remains excluded until its adapter closes. Direct V1 exposes the logical `tokenless/pollinations/sana` model while keeping its private implementation out of the path, request, response, and asset provider identity. Both modes return common authenticated asset URLs and metadata. The real direct gate generated and read back one 768×768 JPEG, and the real Arena gate passed through the built CLI and this endpoint.
