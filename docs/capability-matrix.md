...
Qwen's visible **Select Mode** → **Upload attachment** chooser and physical Markdown card detection are implemented, but `file.upload` remains unadvertised. On 2026-08-09, a detached build reached three visible `.fileitem-btn` cards whose extensions were all `.md`, and the 853-character rendered request contained the frozen 521-character Matrix V2 user prompt verbatim (job `tlp_14ec13d8-aba7-42c6-b328-f9095360d03a`; 14.6 seconds). That historical job was not retried after its send click produced no visible transition. A later run still displayed all three exact cards as `Parsing...` after an unacknowledged submit (job `tlp_e2e_qwen_3980f162-f4ee-4202-b8bc-a4f6c70d2157`; 132.167 seconds), so Qwen acceptance now requires the normalized card-name multiset to show pending processing before the same ready cards can complete a short bounded stability window; any missing card or pending reappearance resets it. The final lifecycle run passed this three-file readiness gate and visible prompt input, then still received no submission acknowledgement (`provider_submitted_at` remained unset; job `tlp_e2e_qwen_aca8d9e6-edf3-4488-8bf7-394bbc157408`; 65.179 seconds). No response or savings event exists, and the route remains unadvertised.
...
Perplexity `conversation.chat` is experimental and routeable. One 2026-08-10 built-CLI and packaged-daemon journey accepted two visible Markdown attachments, submitted the attachment-grounded prompt, and read both exact attachment markers with two normalized visible citations (job `tlp_e2e_DwUfSRhPb0RneFuf_5005733e-edcb-487d-8f12-35e220a3170f`; 13.3 seconds end-to-end). That isolated success does not close the release gate: the later formal two-file selection showed the real visible dialog **Upgrade for additional document analysis** on the cached `signed_in_free` / `Free plan` profile. `file.upload` therefore remains unadvertised; Tokenless reports this visible state as `provider_plan_limited`. Requests above the cached Free-plan two-file boundary are rejected before staging, browser selection, or submission, but the provider's dynamic quota can still block a two-file request. Tokenless makes no claim about paid-plan limits. Continuation, model selection, Deep Research, Spaces, and generated assets remain unadvertised.
...
Dola `conversation.chat` and `file.upload` are experimental signed-in routes from the user-selected managed profile. The chat composer, Fast/Pro model menu, file picker, conversation URLs, and Create Image, Writing, Create Video, Translate, and Homework entry points are visible. On 2026-08-10, the built CLI, packaged daemon, headed `web-ai` profile, and real provider network each accepted three distinct visible Markdown cards (job `tlp_5db59da1-44fd-40ac-b907-340d4f8ef82b`; 5.664 seconds) and three distinct visible Word cards from real `.docx` files (job `tlp_148a7ac2-e104-4e60-b0d3-815e9a3c4039`; 4.989 seconds); both receipts retained the visible acceptance evidence and attachment hashes. The complete three-Markdown attachment-grounded upload, prompt, visible submission, and response journey then passed as job `tlp_24164113-840e-4a53-8645-e9a1268c8438`: 79.614 seconds from creation to completion, including about 64 seconds queued behind prior work, and 10.922 seconds from visible submission to response. The local visible-output estimate recorded 251 tokens for 1,376 characters using `o200k_base`; it is not provider billing. Continuation, generation, and specialist outcomes remain unadvertised.

| Dola control or surface | Canonical outcome candidates | Current evidence and route state |
| --- | --- | --- |
| Chat and same-conversation follow-up | `conversation.chat`, `conversation.continue` | Experimental chat route; the three-file attachment-grounded submission and correlated response gate passed; continuation remains unproven |
...
| Add file | `file.upload` | Experimental file-selection route; three physical Markdown and three physical Word cards passed through the built CLI and packaged daemon in the selected profile |
