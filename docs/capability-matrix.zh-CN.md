...
Authenticated `POST /v1/images/generations` endpoint 是 browser 与 direct execution 共用的 canonical image input。Browser auto routing 会把 enabled、usable provider 与 `conversation.chat`、`image.generation`、`artifact.download` 取交集；Gemini 在 adapter 闭环前保持排除。Direct V1 公开 logical `tokenless/pollinations/sana` model，同时不在 path、request、response 或 asset provider identity 中暴露 private implementation。两种 mode 都返回共有 authenticated asset URL 与 metadata。真实 direct gate 已生成并回读一张 768×768 JPEG，真实 Arena gate 也已通过 built CLI 与该 endpoint。
