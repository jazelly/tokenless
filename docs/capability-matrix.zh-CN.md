...
可选开启的本地 [API proxy](api-proxy-integration.zh-CN.md) 覆盖解析出的 profile 上任何已启用 provider 的 `conversation.chat`，model 命名为 `tokenless/<provider>`。其 OpenAI Chat Completions route 已实现一个非流式现代 function call 的 request boundary 与 prompt protocol：Tokenless 校验 catalog 与配对历史，调用方仍是唯一 tool executor。两次重复的真实 DeepSeek invalid-final JSON escaping output 只为 nonce-correlated `kind: final` envelope 准入一次 same-strategy bounded correction；framing、tool-call、transport 与 tool execution failure 均不重试。Packaged daemon 已通过真实 DeepSeek browser route 完成 single-tool path：job `8a709343-5fd4-46b4-801c-434c5b4a8da0` 返回标准 `tool_calls`，调用 `read_file` 读取 `package.json`；调用方将实际本地结果作为配对的 `role: tool` 历史返回；job `8a3d2c42-e05c-478f-8518-22acb5a39467` 返回基于 package metadata 的最终回答，`finish_reason: stop`。流式与多调用、强制 tool choice、结构化最终输出、Responses 和 Anthropic tool use 仍不公开。Comparison、Search、Image、Code、Agent 与 Video 等 specialized outcome 继续使用 task API，以保留结构化输出。
