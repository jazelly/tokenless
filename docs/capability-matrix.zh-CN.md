...
可选开启的本地 [API proxy](api-proxy-integration.zh-CN.md) 只覆盖 `conversation.chat`，适用于解析出的 profile 上任何已启用的 provider，model 命名为 `tokenless/<provider>`。Comparison、Search、Image、Code、Agent 与 Video 等 specialized outcome 继续使用 task API，以保留结构化输出。
