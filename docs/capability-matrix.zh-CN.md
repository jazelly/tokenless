...
G4F direct `images/generations` route 现已闭环自己的 downloaded-asset boundary。请求需要提供 `tokenless.taskId` 与 `tokenless.conversationId`；Tokenless 会在 upstream call 前移除该本地 context，读取返回的 private media bytes，校验 JPEG/PNG 尺寸，以 `0600` 权限保存 asset，并返回相对 authenticated `/v1/asset/...` URL 与共有 artifact metadata。真实 `PollinationsImage` / `sana` gate 已生成并回读一张 768×768 JPEG。该 direct endpoint 不属于上方 browser-provider capability 表。
