...
可选开启的本地 [API proxy](api-proxy-integration.zh-CN.md) 覆盖解析出的 profile 上任何已启用 provider 的 `conversation.chat`，model 命名为 `tokenless/<provider>`。其 OpenAI Chat Completions route 已实现现代 function calls，可用非流式或终态 SSE 返回：Tokenless 校验 catalog 与完整的一对一 result history，调用方仍是唯一 tool executor。`tool_choice` 支持 auto、none、required 与一个精确 named function；省略/true `parallel_tool_calls` 允许按 model order 返回多个调用，false 最多允许一个，而 named choice 始终恰好返回一个。`strict: true` 只准入递归 closed object schema，要求每个 property 都是 required，并对返回 arguments 做 schema 校验。Framing、choice、call-count 或 schema violation 会以 `provider_output_protocol_error` 失败且不会获得 correction。两次重复的真实 DeepSeek invalid-final JSON escaping output 只为 nonce-correlated 且当前允许的 `kind: final` envelope 准入一次 same-strategy bounded correction；framing、tool-call、transport 与 tool execution failure 均不重试。Packaged daemon 已通过真实 DeepSeek browser route 完成非流式 single-tool path：job `8a709343-5fd4-46b4-801c-434c5b4a8da0` 返回标准 `tool_calls`，调用 `read_file` 读取 `package.json`；调用方将实际本地结果作为配对的 `role: tool` 历史返回；job `8a3d2c42-e05c-478f-8518-22acb5a39467` 返回基于 package metadata 的最终回答，`finish_reason: stop`。后续一次[真实且未修改 DSH 的 streaming run](evidence/dsh-streaming-tool-loop-2026-08-15.md)从稳定终态 SSE delta 重建了两个连续的单调用，由 DSH 执行两个 tool，并通过真实 DeepSeek browser route 获得 grounded final answer。 随后一次[未修改 DSH 的 multiple-call run](evidence/openai-multiple-tool-calls-deepseek-2026-08-15.md)在同一 SSE outcome 中重建了稳定的 index 0/1 调用，执行两个真实读取、重放两项结果并获得 grounded final；另一个真实非流式 case 保留了与两个调用同时返回的 assistant content。结构化最终输出、Responses 和 Anthropic tool use 仍不公开。Comparison、Search、Image、Code、Agent 与 Video 等 specialized outcome 继续使用 task API，以保留结构化输出。
