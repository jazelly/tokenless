...
| GitHub Copilot | Signed-in GitHub.com chat, model selection, Markdown/code-file upload, conversation history, repository context, Spaces, and cloud agents | Experimental browser chat, continuation, citations, and Harness-verified `file.upload` plus `document.input`. On 2026-09-05, the configured Copilot Pro profile passed the 26,438ms Harness gate with GPT-5.6 Luna: two Markdown uploads, a real read-only workspace tool, same-conversation continuation, exact final proof, and no fallback. GPT-5.4 mini passed file/content/citation continuation; Auto was not used for Harness acceptance. Spaces and cloud-agent actions remain unadvertised |
