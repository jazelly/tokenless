...
- Z.ai passed its real non-submission and mutation journeys through the built CLI, packaged daemon, runtime-bound Cloak profile, and provider network. Guest continuation, readiness, prompt drafting, completed response, and conversation mapping are closed; `conversation.chat` is advertised as experimental. The configured entry point is now `https://z.ai/chat`; the official page currently prepares a draft and hands it to the `chat.z.ai` runtime, so the navigation catalog approves both origins. Prior acceptance covered that runtime through a strict E2E-only persisted resolver mapping; the entry-to-runtime journey remains a manual release rerun.
