...
| Credits / quota blocker | The rebuilt response boundary classifies the real Agnes answer as `provider_credits_exhausted` with family `plan_limit` and proof `visible-agnes-insufficient-credits-text`; routing records capacity with `rateLimited: false`. On 2026-09-15, yellow `web-ai` live Usage showed `Free`, 1,200 time-sensitive credits, Permanent 0, and `Daily Free Credits +1200`. This confirms the observed daily refresh for that account/date, not a universal fixed model multiplier or per-message price |
...
| Yellow Terminal-Bench rerun | The same official task ran once with fixed `tokenless/agnes` and explicit `web-ai` on 2026-09-15 (`08-wal-recovery-ordering-agnes-yellow-20260915`): reward 0, 44/97 verifier checks passed, no retry. The first provider job submitted successfully but returned a real permission-error answer; the correction job ended `prompt_submit_failed`; `rateLimited: false`, no tool result or child chain. The refreshed 1,200-credit balance did not produce an insufficient-credit response in this run |
