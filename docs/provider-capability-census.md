...
- `image.generation` and `artifact.download`: experimental ChatGPT, Grok, Arena, and Meta AI;
...
| Dola | Signed-in web chat; Fast and Pro choices; file input; Create Image, Writing, Create Video, Translate, and Homework entries; a separate Seedream image-creation surface; no observed Project, file library, or persistent knowledge-management surface | The real Seedream 4.5 image surface is visible, but this selected browser context requires login before generation. Built-product generation closure remains pending, so no canonical route is advertised |
