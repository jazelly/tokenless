...
- 统一 `POST /v1/images/generations` 现在同时承载 browser 与 direct 图片生成。Direct 对外使用 logical `tokenless/pollinations/sana`，内部私有实现不进入 public path、request、response 或 asset provider identity；真实 gate 已生成一张 768×768 JPEG，并验证 `0600` 落盘与 daemon byte-for-byte readback。
