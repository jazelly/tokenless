...
- ChatGPT's browser image slice is experimental `image.generation` and `artifact.download`: the reader selects only the latest `section[data-turn="assistant"]`, deduplicates repeated image elements by canonical URL, downloads same-origin authenticated bytes through the selected browser session, persists verified assets, and exposes daemon readback. The focused real-provider gate produced one deduplicated 1254×1254 PNG and closed DOM/disk/digest/readback evidence.
- Grok's independent Imagine Image surface is experimental `image.generation` and `artifact.download`: the adapter selects exact ×2 output, records the pre-submit post set, waits for exactly two new terminal post identities, ignores data-URI previews, opens each current post, and persists only its asset-ID-matched HTTPS image. The focused real-provider gate produced two 768×1152 JPEGs and closed baseline set-difference, post identity, provider bytes, local digest, browser decoding, and authenticated readback evidence.
- G4F direct image generation now requires explicit `tokenless.taskId` and `tokenless.conversationId`, strips that local context before the upstream call, downloads the private media result, and returns a relative authenticated Tokenless asset URL plus common metadata. The real `PollinationsImage` / `sana` gate produced a 768×768 JPEG with `0600` persistence and byte-for-byte daemon readback.
- Qwen Create Image / Qwen-Image 2.0 produced one terminal CDN PNG in a real page, but two built-CLI gates failed before action because the configured browser could not resolve `chat.qwen.ai`; the route remains unadvertised. Kimi Design is live-observed but its real submission hit the visible priority-capacity queue. Gemini Images remains pending after provider timeouts despite a real terminal decoded blob image and a verified canvas extraction path.
- Arena is registered as a supported signed-in provider on `https://arena.ai/`. Direct chat, exact model selection, fail-closed continuation, PNG/PDF input, Search with answer-scoped citations, Battle/Side-by-Side comparisons, Image generation/editing, Code website generation, Agent execution, and Battle Video generation are closed through the built CLI, packaged daemon, headed `web-ai` Cloak profile, and real provider network. Experimental task routes preserve citations, alternatives, tool steps, code, image, and video artifacts; Arena Image generation/editing now downloads verified PNG/JPEG/WebP bytes through the selected browser session into task-scoped assets and exposes authenticated daemon readback with local digest evidence. Arena Direct chat is additionally reachable through the shared opt-in local API proxy as `tokenless/arena`. Unproven image-to-video, specialized-surface continuation, and model controls remain unavailable rather than inferred.
- Meta AI is registered as an experimental signed-in provider on `https://meta.ai/`. The built CLI, packaged daemon, selected `web-ai` Cloak profile, and real provider network closed readiness, prompt drafting, exact Instant/Thinking selection, visible text-file acceptance, a substantive completed response, durable `/prompt/:conversationId` mapping, latest-assistant image extraction, browser-session image download, verified local asset persistence, and authenticated daemon readback. The real asset run produced a 1600×1600 PNG. `conversation.chat`, text-file `file.upload`, `image.generation`, and `artifact.download` are routeable; image editing, citations, and explicit continuation remain unadvertised.
...
| Image generation | Experimental, routeable | The latest `[data-testid="assistant-message"]` image tile is read after the stop control disappears; authenticated browser-session bytes are browser-decoded, persisted under task/conversation/job/time-scoped assets, and exposed through authenticated daemon readback |
