...
3. ask whether to enable Anti-Detect mode, identify CloakBrowser as the underlying browser, link to the official CloakBrowser project, and show the exact platform-supported artifact and browser versions; an affirmative answer selects Cloak, while a negative answer continues to ordinary browser selection;
...
5. map each candidate to its owning browser executable version, classify exact four-component alignment against the platform Cloak catalog entry, and show both aligned and non-aligned candidates without reading browser secrets;
6. ask whether to create and open a clean Cloak-bound profile; do not offer Tokenless-managed profile or authentication-state import;
7. resolve or install the selected runtime;
8. verify the resolved runtime and provision or select a clean, compatible managed profile without copying another browser profile;
9. explain that the user signs in inside the managed profile and that its browser-managed session persists across jobs;
