...
Lifecycle note（2026-08-17）：Universal API 的 `/v1/responses` 与 `/v1/openai/responses` 现在为每个响应预生成 `resp_*`，并将其作为 managed provider task identity；因此无需 ledger schema migration 即可在同一 browser provider chat 上继续。Chat Completions 与 Anthropic 始终按 stateless contract 新建 provider chat 并发送完整 history；Responses 省略 `previous_response_id` 时同样新建并发送完整 input，只有 valid、route-compatible ledger 且存在 task mapping 时才打开 canonical provider URL 并发送当前 delta。Structured/tool continuation 只编译当前 tool result/message 与当前 catalog；ledger 存在但 mapping 缺失时安全降级为完整 transcript 的新 chat。24 小时/1,000-entry local ledger 仍不保存 opaque reasoning 或 browser session。现有真实 DeepSeek evidence 覆盖 function call、caller-owned result、full-input replay、`previous_response_id` continuation、function/text stream reconstruction 与 structured final；证据见 [OpenAI Responses — DeepSeek](../evidence/openai-responses-deepseek-2026-08-15.md)。TC-007 在 fresh reviewer 与 coordinator verification 通过前保持 active。
