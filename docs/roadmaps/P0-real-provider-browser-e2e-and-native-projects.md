...
The repository no longer stores or tests against provider DOM captures. Selector, parser, blocker, and transition development uses the real provider website through the configured persistent browser profile.
...
The previous complete manual gate attempts produced:
...
All three gates ran with zero skipped cases and no internal retry. This predates the narrow Claude Cloudflare known-issue skip classification now declared in the matrix; the release bar remains zero skips except for that exact durable blocker condition. Gemini and Qwen provided real mutation closure for every capability required by their matrix entries at that time. That earlier mutation run passed Gemini submit/read, citations, and conversation workspace plus Qwen submit/read and conversation workspace. It failed all ChatGPT, Claude, and Grok cases with `e2e_provider_auth_unavailable` because the selected setup-managed profile was not authenticated for those providers.

On 2026-07-31, the mutation cases were consolidated around provider submissions rather than capability-by-capability messages. ChatGPT, Claude, and Grok now prove attachment acceptance, response reading, citations, conversation fallback, durable mapping, and cross-process continuation in one two-turn workflow. Gemini proves response reading, citations, conversation fallback, and durable mapping in one submission. Qwen proves Deep Research mode selection, response reading, conversation fallback, and durable mapping in one submission. The matrix records and the harness enforces an exact per-case submission budget. The full manual gate budget is now 12 submissions: 8 in the mutation gate and 4 in the two native Project cases, down from 28. This structural reduction is not provider acceptance evidence; the consolidated cases required a fresh manual real-provider run.

A fresh same-machine run on 2026-07-31 then produced:

- non-submission: 17 invoked cases, 6 passed and 11 failed;
- mutation: 5 invoked cases, 1 passed and 4 failed; and
- Project: 2 invoked cases, 0 passed and 2 failed.

All three gates ran with zero skipped cases and no internal retry. Qwen resolved successfully and passed all three non-submission cases, while Gemini passed both non-submission cases and its real mutation workflow. ChatGPT, Claude, and Grok remained unauthenticated in the selected setup-managed profile. ChatGPT additionally failed visible attachment observation, Claude model inspection encountered a navigation-destroyed execution context, and Qwen mutation stopped before submission because the independent observer did not see Deep Research Advanced selected. These results replace DNS instability as the current Qwen blocker without treating either failure as acceptance evidence.
...
### Phase 1: Live Development Evidence

Observe and exercise these Claude and Grok states on the real websites through the configured persistent profile:
...
Use bounded live observer assertions to guide provider-owned selectors and parsers without storing DOM snapshots or replicas.

Exit: implementation and tests prove the required states and transitions on the real provider websites.
...
- The repository contains no provider DOM fixtures, provider replicas, or tests that intercept provider routes.
...
| Risk | Response |
| --- | --- |
| Provider UI drift | Fail the required live case, inspect the changed real site through bounded diagnostics, and update the provider-owned adapter and live case |
