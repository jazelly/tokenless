...
Updates provider membership, Chromium color, and visibility for one profile with `--profile`:

```bash
tokenless config \
  --profile work \
  --profile-color '#0B57D0' \
...
- `--language <en|zh-CN>`
- `--profile <slug> --profile-color '#RRGGBB'`
...
`profileColor` applies only to a managed Chromium profile with its own browser identity. Tokenless passes the configured RGB value through Chromium's `--install-autogenerated-theme` support; changing it closes only that selected profile after confirming it has no pending browser jobs, and the next open uses the new color.
...
- Tokenless merges explicit capabilities with structural inference: a normal `submit_and_read` run requires `conversation.chat`, `--attach-file` requires `file.upload` plus `document.input` for non-media files or the matching `image.input`, `audio.input`, or `video.input`, and each explicit semantic input requires a matching MIME attachment. `--workspace-mode auto` or `native` requires `workspace.native`.
