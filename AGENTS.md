...
- An invoked E2E suite must never skip a required case and must not retry internally. Missing activation, unavailable authentication, provider failure, blocker state, or another unmet prerequisite must fail with a clear reason. Retry only by manually running the suite again.
