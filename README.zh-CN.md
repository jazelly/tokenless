...
让任何智能体通过统一的本地命令行操作 AI 网页。Tokenless 为 ChatGPT、Claude、Gemini 和 Grok 提供一套与服务商无关的流程；公开的本地接口仍在开发中。
...
- **一套接口调用四家服务商。** 命令行统一调用四家服务商，文件、工具、图片和多模态能力也将接入同一套协议。
...
按照交互式命令行选择浏览器和一个现有 Chrome 或 Brave 配置。获得明确同意后，Tokenless 会把 ChatGPT、Claude 和 Grok 中受支持的登录状态复制到独立托管配置；Gemini 和共享的 Google 登录数据不会被导入。原浏览器配置保持不变。
...
首次安装时，此命令会创建干净的 `default` 配置、启动本地运行环境，并检查每个受支持的可见服务商，在需要时打开登录页面。它不会隐式导入现有浏览器配置。
...
- 在可见页面支持的范围内开放服务商工作区、文件、插件、连接器和工具。
...
`智能体 → 命令行 → 本地守护进程 → Playwright 运行进程 → 托管浏览器配置 → 服务商网页`

命令行提交统一操作，本地守护进程负责调度，Playwright 使用所选托管配置操作网页，各服务商适配器验证可见结果。后续任务会复用同一配置。规划中的本地接口会在认证和兼容性边界稳定后，复用同一套应用与任务协议。
...
| 本地接口 | 执行同一套与服务商无关的 Playwright 任务 | 规划中；尚不是公开兼容性入口 |

## 隐私与安全

- Playwright 在本机运行，并使用可见、可长期复用的托管浏览器配置。
- 经用户同意的导入只复制受支持服务商的登录状态，不包含密码、历史记录、书签、支付、同步和缓存数据。
- 自动化只操作可见网页控件，并检查可见结果。
- 验证码、登录、套餐限制、授权和确认步骤仍由用户控制。
- 所选文件会在本机暂存并校验完整性，再通过服务商的可见文件控件上传。
- 每个请求都通过经过认证的本地守护进程和托管 Playwright 路径执行。
