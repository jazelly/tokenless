...
| 模式 | 传输路径 | 身份认证 | 状态 |
| --- | --- | --- | --- |
| 扩展模式（`visible`，默认） | ChatGPT、Claude、Gemini 或 Grok 可见网页界面 | 浏览器登录会话 | **推荐** |
...
可见模式通过打包的 Rust daemon 和 `tokenless-native-host` 将 CLI 连接到浏览器扩展；它不会调用隐藏的 provider 后端接口，也不存在本地 JSON 或本地任务页回退路径。
...
- **可持续扩展。** 当前可见网页适配器支持 ChatGPT、Claude、Gemini 和 Grok，也可扩展至其他具有兼容网页界面的 AI 服务商。
...
- 默认服务商为 ChatGPT，同时支持 Claude、Gemini 和 Grok。
