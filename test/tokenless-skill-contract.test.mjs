...
test('tokenless-install skill treats upgrade as the canonical maintenance path', async () => {
  ...
  assert.match(installSkill, /command itself is prompt-free/)
  assert.match(installSkill, /Human users may omit the flag and run `tokenless upgrade`/)
  assert.match(installSkill, /Agents and CI keep `--json`/i)
  assert.match(installSkill, /not a different upgrade workflow or a version selector/)
