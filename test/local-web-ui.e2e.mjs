...
test('Svelte Web UI completes setup, persists configuration, renders durable work, and remains responsive', async () => {
  await withDaemon(async ({ daemon, homeDir }) => {
    ...
    try {
      ...
      assert.equal(new URL(page.url()).pathname, '/dashboard/profiles/')
      ...
      assert.equal(new URL(page.url()).pathname, '/dashboard/overview/')
      assert.match(await page.getByTestId('top-header').textContent(), /tokens saved|Runtime|Idle|Waiting jobs|Finished jobs|Quick settings/)
      assert.equal(await page.getByTestId('header-runtime-status').textContent(), 'Idle')
      assert.equal(await page.getByTestId('header-runtime-status').getAttribute('data-state'), 'idle')
      assert.equal(await page.getByTestId('header-finished-jobs').locator('strong').textContent(), '0')
      assert.equal(await page.getByTestId('header-tokens-saved').evaluate((element) => getComputedStyle(element).flexDirection), 'row')
      assert.equal(await page.locator('.top-header-actions a[data-dashboard-section="system"]').count(), 0)
      assert.equal(await page.getByTestId('quick-settings-trigger').evaluate((element) => element.tagName), 'BUTTON')
      await page.getByTestId('quick-settings-trigger').click()
      await page.getByTestId('quick-settings-panel').waitFor()
      assert.match(await page.getByTestId('quick-settings-panel').textContent(), /Language|English|简体中文|Output savings/)
      const quickChineseSaved = page.waitForResponse((response) => (
        new URL(response.url()).pathname === '/dashboard-api/v1/config' &&
        response.request().method() === 'PATCH'
      ))
      await page.getByTestId('quick-settings-panel').getByRole('button', { name: '简体中文' }).click()
      assert.equal((await quickChineseSaved).status(), 200)
      await page.waitForFunction(() => document.documentElement.lang === 'zh-CN')
      assert.equal(JSON.parse(fs.readFileSync(path.join(homeDir, 'config.json'), 'utf8')).language, 'zh-CN')
      const quickEnglishSaved = page.waitForResponse((response) => (
        new URL(response.url()).pathname === '/dashboard-api/v1/config' &&
        response.request().method() === 'PATCH'
      ))
      await page.getByTestId('quick-settings-panel').getByRole('button', { name: 'English' }).click()
      assert.equal((await quickEnglishSaved).status(), 200)
      await page.waitForFunction(() => document.documentElement.lang === 'en')
      await page.keyboard.press('Escape')
      assert.equal(await page.getByTestId('quick-settings-panel').count(), 0)
      assert.equal(await page.getByTestId('quick-settings-trigger').evaluate((element) => document.activeElement === element), true)
      await page.getByTestId('analytics-kpis').waitFor()
      await page.getByTestId('provider-panorama').waitFor()
      await page.getByTestId('analytics-cumulative-chart').waitFor()
      await page.getByTestId('analytics-outcomes-chart').waitFor()
      await page.getByTestId('capability-usage-matrix').waitFor()
      await page.getByTestId('capability-mix-chart').waitFor()
      await page.getByTestId('execution-mode-mix').waitFor()
      assert.match(await page.getByTestId('overview-view').textContent(), /Usage analytics|Provider Panorama|No provider usage in this range/)
      assert.equal(await page.getByTestId('analytics-range-30d').getAttribute('aria-pressed'), 'true')
      await page.getByTestId('analytics-range-7d').click()
      await page.waitForFunction(() => document.querySelector('[data-testid="analytics-range-7d"]')?.getAttribute('aria-pressed') === 'true')
      assert.match(await page.getByTestId('analytics-kpis').textContent(), /Selected range/)
      ...
      const headerProfile = page.getByTestId('header-profile')
      await headerProfile.click()
      assert.deepEqual(await page.getByRole('option').evaluateAll((options) => options.map((option) => option.dataset.profile).sort()), ['personal', 'work'])
      await page.getByRole('option').filter({ hasText: 'work' }).click()
      assert.equal(new URL(page.url()).searchParams.get('profile'), 'work')
      await headerProfile.click()
      ...
      const work = daemon.store.createJob({
        ...
        request_json: {
          ...
          context: { requirements: ['conversation.chat'] },
      ...
      await page.waitForTimeout(3300)
      await activateNavigation(page, 'overview')
      const chatgptPanorama = page.getByTestId('provider-panorama-chatgpt')
      await chatgptPanorama.waitFor()
      assert.match(await chatgptPanorama.textContent(), /ChatGPT|1/)
      assert.match(await page.getByTestId('analytics-kpis').textContent(), /ChatGPT|1/)
      assert.match(await page.getByTestId('capability-usage-matrix').textContent(), /ChatGPT|Conversation/)
      assert.match(await page.getByTestId('execution-mode-mix').textContent(), /Browser · 1/)
      ...
      assert.equal(fs.existsSync(path.join(homeDir, 'tokenizers')), false)
      await page.getByTestId('quick-settings-trigger').click()
      assert.equal(await page.getByTestId('quick-output-savings').getAttribute('aria-checked'), 'true')
      await page.getByTestId('quick-output-savings').click()
      await page.waitForFunction(() => document.querySelector('[data-testid="output-savings-card"]')?.textContent?.includes('disabled'))
      assert.equal(await page.getByTestId('quick-output-savings').getAttribute('aria-checked'), 'false')
      await activateNavigation(page, 'overview')
      const cumulativeChart = page.getByTestId('analytics-cumulative-chart')
      await cumulativeChart.waitFor()
      assert.match(await cumulativeChart.textContent(), /Cumulative output savings|No measured savings in this range/)
      assert.equal(fs.existsSync(path.join(homeDir, 'tokenizers')), false)
      await activateNavigation(page, 'system')
      ...
      await activateNavigation(page, 'overview')
      assert.match(await page.getByTestId('overview-view').textContent(), /使用分析|Provider 全景|累计输出节省/)
      assert.match(await page.getByTestId('capability-usage-matrix').textContent(), /Provider × Capability 矩阵/)
      await page.reload({ waitUntil: 'networkidle' })
      await page.getByTestId('app-shell').waitFor()
      await activateNavigation(page, 'providers')
