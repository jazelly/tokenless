...
test('Svelte Web UI completes setup, persists configuration, renders durable work, and remains responsive', async () => {
  await withDaemon(async ({ daemon, homeDir }) => {
    ...
    try {
      ...
      const chatgptBrowserMode = page.getByTestId('provider-mode-badges-chatgpt').locator('[data-mode="browser"]').locator('..')
      assert.match(await chatgptBrowserMode.locator('.hover-tooltip-content').textContent(), /^ChatGPT · Browser · Configured · Runs through the signed-in provider website\.$/)
