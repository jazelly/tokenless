...
test('Svelte Web UI completes setup, persists configuration, renders durable work, and remains responsive', async () => {
  await withDaemon(async ({ daemon, homeDir }) => {
    ...
    try {
      ...
      assert.match(await page.getByTestId('top-header').textContent(), /tokens saved|Runtime|Idle|Waiting jobs|Finished jobs|System|v\d+/)
      ...
      assert.equal(await readinessRefresh.getAttribute('title'), null)
      assert.equal(await readinessRefresh.evaluate((element) => element.tagName), 'BUTTON')
      const readinessRefreshTooltip = readinessRefresh.locator('..')
      assert.equal(await readinessRefreshTooltip.locator('.hover-tooltip-content').textContent(), 'Refresh provider readiness')
      await readinessRefreshTooltip.hover()
      await page.waitForTimeout(550)
      assert.equal(await readinessRefreshTooltip.locator('.hover-tooltip-content').evaluate((element) => getComputedStyle(element).opacity), '1')
      const chatgptAuthIndicator = page.getByTestId('provider-access-chatgpt').locator('.provider-access-indicator').first()
      assert.equal(await chatgptAuthIndicator.getAttribute('aria-label'), 'ChatGPT · Never checked')
      assert.equal(await chatgptAuthIndicator.locator('.hover-tooltip-content').textContent(), 'ChatGPT · Never checked')
      const chatgptBrowserMode = page.getByTestId('provider-mode-entry-chatgpt-browser')
      const chatgptBrowserTooltip = chatgptBrowserMode.locator('.hover-tooltip-content')
      assert.equal(await chatgptBrowserMode.evaluate((element) => element.tagName), 'A')
      assert.equal(await chatgptBrowserMode.getAttribute('href'), 'https://chatgpt.com/')
      assert.match(await chatgptBrowserTooltip.textContent(), /^ChatGPT · Browser · Configured · Runs through the signed-in provider website\.https:\/\/chatgpt\.com\/$/)
      await chatgptBrowserMode.hover()
      await page.waitForTimeout(550)
      ...
      assert.deepEqual(await headerProfile.locator('option').evaluateAll((options) => options.map((option) => option.value).sort()), ['personal', 'work'])
      await headerProfile.selectOption('work')
      assert.equal(new URL(page.url()).searchParams.get('profile'), 'work')
      await headerProfile.selectOption('personal')
      assert.equal(new URL(page.url()).searchParams.get('profile'), 'personal')
      ...
      await page.waitForTimeout(3300)
      await activateNavigation(page, 'overview')
      assert.match(await page.getByTestId('overview-recent-jobs-summary').textContent(), /^Showing latest \d+ conversations$/)
      const overviewWorkRow = page.getByTestId(`overview-job-${work.job_id}`)
      await overviewWorkRow.waitFor()
      const overviewWorkUrl = new URL(await overviewWorkRow.getAttribute('href'), consoleOrigin)
      assert.equal(overviewWorkUrl.pathname, '/dashboard/jobs/')
      assert.equal(overviewWorkUrl.searchParams.get('job'), work.job_id)
      await overviewWorkRow.click()
      await page.getByTestId('job-detail-page').waitFor()
      assert.equal(new URL(page.url()).searchParams.get('job'), work.job_id)
      ...
      assert.equal(await disabledSavingsPanel.getByRole('link').getAttribute('href'), '/dashboard/system/?profile=work')
      assert.equal(fs.existsSync(path.join(homeDir, 'tokenizers')), false)
      await activateNavigation(page, 'system')
