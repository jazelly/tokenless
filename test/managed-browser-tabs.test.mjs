  ...
  await withManager(async ({ manager, profile }) => {
    ...
    try {
      ...
      if (profile.profileColor) {
        const rgb = profile.profileColor.match(/[0-9A-F]{2}/gu).map((component) => Number.parseInt(component, 16)).join(',')
        assert.match(commandLine ?? '', new RegExp(`(?:^|\\s)--install-autogenerated-theme=${rgb}(?:\\s|$)`, 'u'))
      }
    } finally {
...
async function withManager(operation) {
  ...
  const profile = {
    ...primary.profile,
    profileColor: primary.config.profiles[primary.profile.slug]?.profileColor,
  }
  try {
