...
test('capabilities list exposes canonical outcomes and only evidence-backed routes', () => {
  ...
  assert.deepEqual(byId.get('image.generation').routes.map((route) => route.provider), ['grok', 'chatgpt', 'meta', 'arena'])
  assert.deepEqual(byId.get('artifact.download').routes.map((route) => route.provider), ['grok', 'chatgpt', 'meta', 'arena'])
