...
test('Tokenless CLI job completes through extension task page and ChatGPT real-DOM fixture', {
  ...
  try {
    ...
    assert.equal(snapshotResult.result.title, '[text]')
    ...
    assert.doesNotMatch(snapshotHtml, /Earlier Tokenless selector check/)
    assert.doesNotMatch(snapshotHtml, /aria-label="New chat"/)
    assert.match(snapshotHtml, /aria-label="\[text\]"/)
    assert.match(snapshotHtml, /href="\[url\]"/)
