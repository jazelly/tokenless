...
`setup` first resolves an exact browser runtime, creates or selects a runtime-bound managed profile, then installs the required agent skills, prepares the local daemon, checks every enabled provider once, and leaves one headed provider review tab open for each enabled provider. Normal setup always uses platform-pinned Tokenless-managed Chrome for Testing: `145.0.7632.6` on Apple Silicon macOS and `146.0.7680.165` on Windows x64. Tokenless downloads and checksum-verifies the official artifact during setup when it is not already cached; the browser binary is not embedded in the npm package. Interactive setup asks whether to use Anti-Detect mode and states that accepting will install the verified platform-pinned CloakBrowser when needed. Declining uses managed Chrome for Testing and a clean profile. Before download, the Cloak flow links to the official project, shows the platform pin, and labels profile import as experimental. Import supports only Google Chrome, may fail by browser version or platform, and does not guarantee sign-in-state transfer. Setup scans only safe directory/version metadata for Google Chrome profiles and classifies exact version alignment; Edge, Chromium, Chrome for Testing, Brave, Arc, and other browser profiles are ineligible. If compatible profiles exist, one source choice offers `Start clean` and those profiles; selecting a profile explicitly authorizes its opaque local copy without a separate import, copy-consent, or installation confirmation. For non-interactive setup, explicit `--anti-detect` or `--browser cloak` authorizes installation; a stored Cloak preference alone fails before download, and import still requires `--consent-local-profile-copy`. Tokenless downloads Cloak from its official release and does not redistribute it. On first setup it also detects `en` or `zh-CN` from the system locale; use `tokenless config --language <en|zh-CN>` to override the saved preference.
...
New profiles are bound to the exact runtime that created them. Normal managed Chrome for Testing profiles always start clean. Experimental opaque Google Chrome profile copy is available only for a new CloakBrowser profile. Tokenless does not inspect or expose individual cookies, tokens, browser storage, or authentication values. Import may fail by browser version or platform, and a successful open does not guarantee sign-in-state transfer. macOS importability parity is recorded for an unauthenticated Chrome for Testing 145 profile opened in both Chrome for Testing 145 and Cloak 145; Windows 146 parity and sign-in-state portability remain unverified, and no managed Chrome-for-Testing-to-Cloak migration is currently exposed.
