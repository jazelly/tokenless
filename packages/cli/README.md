...
New profiles are bound to the exact runtime that created them. They start clean unless the user explicitly selects a version-compatible local Google Chrome profile for experimental opaque filesystem copy. Tokenless does not inspect or expose individual cookies, tokens, browser storage, or authentication values. Import may fail by browser version or platform, and a successful open does not guarantee sign-in-state transfer. The managed browser then keeps any session it can use across jobs.
