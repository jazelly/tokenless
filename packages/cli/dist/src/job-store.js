...
export function normalizeBrowserId(browser) {
...
export async function createLocalJob({ homeDir = tokenlessHome(), provider = 'chatgpt', action = 'submit_and_read', prompt, projectRoot, projectName, chatName, targetUrl, idempotencyKey, readDelayMs = 1000, readTimeoutMs = 120000, metadata, includeText, maxTextChars, ttlMs = 15 * 60 * 1000, } = {}) {
...
export async function readLocalTaskState({ homeDir = tokenlessHome(), taskId, jobId, provider, projectName, chatName, limit = 10, } = {}) {
    ...
    const jobs = (await readJobDetails(homeDir))
        ...
        if (jobId && job.jobId !== jobId)
            return false;
        if (normalizedTaskId && job.taskId !== normalizedTaskId)
            return false;
...
export async function writeDomSnapshot({ homeDir = tokenlessHome(), jobId, nonce, provider, snapshot, } = {}) {
...
export async function writeJobState({ homeDir = tokenlessHome(), jobId, nonce, status, actor, detail, } = {}) {
...
export async function completeLocalJob({ homeDir = tokenlessHome(), jobId, nonce, ok, result, error, actor = 'native-host', } = {}) {
    const request = await readLocalJobRequest({ homeDir, jobId, nonce });
    ...
    if (ok) {
        try {
            ...
            mappingError = localStateError('conversation_map_error', `Failed to persist Tokenless conversation mapping: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
export async function readConversationMap(homeDir = tokenlessHome()) {
    ...
    try {
        ...
        throw localStateError('conversation_map_unreadable', `Cannot read Tokenless conversation map at ${mapPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
export async function readTokenlessConfig(homeDir = tokenlessHome()) {
    ...
    try {
        ...
        throw localStateError('tokenless_config_unreadable', `Cannot read Tokenless config at ${file}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
export async function upsertConversationMapping({ homeDir = tokenlessHome(), provider, idempotencyKey, targetUrl, jobId, projectName, chatName, projectRoot, } = {}) {
...
export async function readLocalHistory({ homeDir = tokenlessHome(), limit = 50 } = {}) {
    ...
    for (const job of await readJobSummaries(homeDir)) {
        if (!job.provider || !job.idempotencyKey)
            continue;
...
export async function waitLocalJobResult({ homeDir = tokenlessHome(), jobId, nonce, timeoutMs = 120000, pollMs = 250, statusIntervalMs = 10000, onStatus, } = {}) {
...
export async function installNativeHost({ homeDir = tokenlessHome(), manifestHome = os.homedir(), extensionId, browsers = ['chrome', 'chrome-for-testing', 'chrome-for-testing-legacy', 'chromium', 'edge', 'arc', 'brave'], nodePath = process.execPath, packageRoot = path.resolve(fileURLToPath(new URL('..', import.meta.url))), } = {}) {
...
async function readJobDetails(homeDir) {
    ...
    for (const file of requestFiles) {
        ...
        if (!request || request.protocol !== LOCAL_JOB_PROTOCOL_VERSION)
            continue;
...
function normalizeProviderList(providers) {
    ...
    for (const provider of providers) {
        ...
        if (!SUPPORTED_PROVIDER_IDS.includes(value) || seen.has(value))
            continue;
        seen.add(value);
