...
export async function readTokenlessConfig(homeDir = tokenlessHome()) {
    ...
    try {
        ...
        throw configError('tokenless_config_unreadable', `Cannot read Tokenless config at ${file}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
export function nativeMessagingHostDir(browser, home = os.homedir(), platform = process.platform) {
    return nativeMessagingHostDirs(browser, home, platform)[0] ?? null;
}
export function nativeMessagingHostDirs(browser, home = os.homedir(), platform = process.platform) {
    const browserId = normalizeBrowserId(browser);
    if (!browserId)
        return [];
    if (browserId === 'profile')
