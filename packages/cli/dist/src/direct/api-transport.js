...
export async function postDirectJson(request) {
    ...
    try {
        ...
        const requestId = responseRequestId(response.headers, request.requestIdHeaders, request.authentication.apiKey);
        ...
        if (!response.ok) {
            ...
            throw upstreamHttpError(response.status, errorText.text, errorText.truncated, request.authentication.apiKey, requestId ?? bodyRequestId);
        }
...
export function normalizeResponseRequestId(value, apiKey) {
...
function parseResponseObject(text, requestId) {
    let parsed;
    try {
...
function upstreamHttpError(status, body, bodyTruncated, apiKey, requestId) {
    ...
    const label = code === 'direct_authentication_failed'
...
function errorResponseRequestId(body, apiKey) {
    try {
        const parsed = JSON.parse(body);
        if (isRecord(parsed))
            return normalizeResponseRequestId(parsed.request_id, apiKey);
    }
    catch {
...
async function readBoundedBody(response, limit, operation) {
...
function createAbortOperation(timeoutMs, externalSignal) {
