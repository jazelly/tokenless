...
async function startRun(ctx, config, request) {
  ...
  const delegatedPrompt = Object.keys(benchmarkEnvironment).length > 0
    ? `${prompt}\n\nBenchmark: First batch = exactly one workspace.search call; no Skill loads/user input. Query \"tokenless_harness_workspace_probe\", path \".\" proves delegated-workspace execution. Query is a read-only ripgrep regex, never shell/listing; no git, ls, find, or shell command in query. workspace.read paths are relative. For VCS, read .git/HEAD, .git/logs/HEAD, and relevant refs/reflogs; never run Git via workspace.search. Then exactly one comprehensive read-only batch with all needed workspace.read/search calls. Then a short plain-text task-relevant final; no further calls, Skill loads/user input. If incomplete, give bounded analysis, not another batch. Probe is not completion. Keep final free of quotes, backticks, braces, backslashes for JSON.`
    : prompt
