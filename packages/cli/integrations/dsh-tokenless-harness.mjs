...
async function startRun(ctx, config, request) {
  ...
  const delegatedPrompt = Object.keys(benchmarkEnvironment).length > 0
    ? `${prompt}\n\nBenchmark integration requirement: the first action batch must contain exactly one call and no Skill loads or user-input needs. That one call must be workspace.search with query \"tokenless_harness_workspace_probe\" and path \".\". Do not include any other call in the first batch. This is a required read-only proof that the child Tokenless Harness executed inside the delegated workspace. A workspace.search query is a ripgrep content regular expression, not a shell command or directory listing; never put git, ls, find, or another shell command in query. A workspace.read path is a relative file path. For a version-control task, read concrete metadata such as .git/HEAD, .git/logs/HEAD, and relevant refs or reflogs instead of trying to execute Git through workspace.search. After receiving the probe result, make exactly one comprehensive read-only action batch using the available workspace.read and workspace.search tools to gather the best evidence for the original delegated task. Include every relevant read and search you need in that batch. After receiving that batch result, return a short plain-text task-relevant final response with no further tool calls, Skill loads, or user-input needs. Keep that final response free of quotation marks, backticks, braces, and backslashes so it remains valid inside the required JSON response envelope. If the available read-only evidence is incomplete, return the best bounded analysis you can instead of requesting another action batch. Do not treat the workspace probe as task completion.`
    : prompt
