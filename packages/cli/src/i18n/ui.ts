...
const zhMessages: Record<keyof typeof enMessages, string> = {
    ...
    routerNeedsModels: '请至少添加一个候选模型。', routerConfigInvalid: '每个模型都需要唯一且有效的 ID、标签和适合的任务。', routerTaskRequired: '请输入任务 prompt。', routerApiUnsupported: '当前 Chrome 没有暴露所选的 Router Engine。', routerApiUnavailable: '当前页面或设备无法使用所选的 Router Engine。', routerInvalidResult: '所选 Router Engine 返回了无效的模型选择。',
    routerTest: '语义路由实验', routerTestHelp: '固定的语义路由指令只接收任务和候选模型配置，不包含 provider 或 operational 数据。', taskPrompt: '任务 prompt', taskPromptPlaceholder: '比较三辆二手车，并总结性价比最高的一辆。', runSemanticRouter: '运行语义路由', routerRunning: '路由中…', routerResult: '结构化结果', downloadProgress: '模型下载', downloadZeroHelp: '0% 表示 Chrome 已接受下载请求，但尚未报告进一步的模型进度。请查看下方的 Model Status 和 Event Logs。',
