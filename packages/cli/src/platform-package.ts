...
type NativeTuple = keyof typeof NATIVE_PLATFORM_PACKAGES
...
type PlatformPackageError = Error & {
  code?: string
  retryable?: boolean
}
...
export function nativePlatformPackageName(
  ...
  const tuple = `${platform}-${arch}` as NativeTuple
  const packageName = NATIVE_PLATFORM_PACKAGES[tuple]
  if (packageName) return packageName
  throw platformPackageError(
    'unsupported_native_platform',
    `Tokenless has no native runtime for ${platform}-${arch}. Supported platforms: ${Object.keys(NATIVE_PLATFORM_PACKAGES).join(', ')}.`
  )
}

export function resolveNativePlatformPackage({
  platform = process.platform,
  arch = process.arch,
  expectedVersion = tokenlessPackageVersion(),
  resolvePackageJson = defaultResolvePackageJson,
}: ResolveNativePlatformPackageOptions = {}) {
  ...
  try {
    ...
    throw platformPackageError(
      ...
      `The optional native runtime ${packageName}@${expectedVersion} is not installed. Reinstall tokenless with optional dependencies enabled. Tokenless never downloads executables at runtime.`
    )
  }

  let manifest: NativePackageManifest
  try {
    manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8')) as NativePackageManifest
  } catch {
    throw platformPackageError(
      'native_platform_package_invalid',
      `Cannot read the installed native runtime manifest at ${manifestPath}. Reinstall tokenless.`
    )
  }
  ...
  if (!valid) {
    throw platformPackageError(
      'native_platform_package_invalid',
      `Native runtime ${packageName} does not match tokenless@${expectedVersion} for ${platform}-${arch}. Reinstall tokenless.`
    )
  }
  return {
    name: packageName,
...
function platformPackageError(code: string, message: string) {
  const error = new Error(message) as PlatformPackageError
  error.code = code
