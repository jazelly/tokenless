...
export function validateDirectApiRequest(
  request: DirectRunRequest,
  expectedProvider: DirectProvider,
  maximumTemperature = 2,
): ValidatedDirectApiRequest {
  ...
  if (request.provider !== expectedProvider) {
    throw new DirectError(
      ...
      `The ${expectedProvider} API adapter only supports provider ${expectedProvider}.`,
    )
  }
  ...
  if (
    ...
    throw new DirectError(
      ...
      `temperature must be a finite number between 0 and ${maximumTemperature}.`,
    )
  }
...
export function optionalNonnegativeInteger(record: Record<string, unknown>, key: string) {
  const value = record[key]
  if (value === undefined || value === null) return undefined
  return nonnegativeInteger(value) ?? INVALID_NONNEGATIVE_INTEGER
}

export function safeNonnegativeSum(values: readonly (number | undefined)[]) {
