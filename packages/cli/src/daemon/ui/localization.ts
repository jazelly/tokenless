...
const messages = {
  en: {
    ...
    outputSavings: 'Output savings', outputSavingsLede: 'Optionally estimate visible assistant output locally. Disabled means no tokenizer is installed or run.',
    ...
    lazyTokenizerDownload: 'The pinned WASM tokenizer is downloaded only when you explicitly enable this feature. It uses CPU only during short measurements and never requires a GPU.',
    enableOutputSavings: 'Enable and download', disableOutputSavings: 'Disable measurement', clearSavingsHistory: 'Clear history', uninstallTokenizer: 'Uninstall tokenizer',
  ...
  'zh-CN': {
    ...
    outputSavings: '输出节省', outputSavingsLede: '可选地在本机估算可见 assistant 输出；停用时不会安装或运行 tokenizer。',
    estimatedTokensSaved: '估算节省的 output token', measuredResponses: '已计量响应', tokenizerRuntime: 'Tokenizer 运行时', runtimeSize: '安装大小',
    lazyTokenizerDownload: '固定版本的 WASM tokenizer 只会在你明确启用后按需下载。它仅在短时计量期间使用 CPU，不需要 GPU。',
    enableOutputSavings: '启用并下载', disableOutputSavings: '停用计量', clearSavingsHistory: '清空历史', uninstallTokenizer: '卸载 tokenizer',
