...
const messages = {
  ...
  'zh-CN': {
    ...
    configPersisted: '配置已持久化。', setupIncomplete: 'Setup 尚未保存配置 yet.', browserReady: '浏览器运行时可用。', browserUnavailable: '浏览器运行时不可用。', noManagedProfiles: '尚未配置 managed profile。', profilesRegistered: '个 managed profile 已注册。', activeBrowserJobs: '个浏览器任务正在运行。',
  },
} as const
...
const uiErrorZh: Record<string, string> = {
  ...
  browser_profile_copy_disabled: 'Tokenless 不复制本地浏览器 profile 或认证状态；请创建全新 managed profile，并在其中手动登录。',
