...
const messages = {
  en: {
    ...
    cleanProfileNote: 'Normal profiles start clean in managed Chrome for Testing. Experimental opaque Google Chrome profile import is available only when creating a CloakBrowser profile.',
    ...
    profileStart: 'Start from', cleanProfile: 'Clean profile', cleanProfileHelp: 'Sign in in a new isolated browser.', copyProfile: 'Copy existing profile', copyProfileHelp: 'Copy one local Google Chrome profile as an opaque tree.', copyChromeProfileExperimental: 'Copy Google Chrome profile (experimental)', copyChromeProfileExperimentalHelp: 'Try an opaque copy from Google Chrome; some versions or platforms may not work.',
    profileImportExperimentalHelp: 'Experimental: only Google Chrome is supported. Edge, Chromium, Chrome for Testing, Brave, Arc, and other browsers are not eligible. A profile opening successfully does not guarantee that sign-in state transfers.',
    browserProfiles: 'Local Google Chrome profiles', profileScanPrivacy: 'Only names and versions are shown here.', scanProfiles: 'Scan', scanAgain: 'Scan again', sourceProfile: 'Source profile', sourceBrowser: 'Browser', profileRootPath: 'Profile root', chromeProfileRootPath: 'Google Chrome profile root', profileRootPathPlaceholder: '/absolute/path/to/user-data…', customProfileRoot: 'Custom profile root', scanProfilesHelp: 'Scan when you want to select an existing Google Chrome profile.', incompatible: 'Incompatible',
  ...
  'zh-CN': {
    ...
    cleanProfileNote: '普通 profile 会在 managed Chrome for Testing 中从 clean 状态开始。只有创建 CloakBrowser profile 时，才提供实验性的 Google Chrome opaque profile 导入。',
    ...
    profileStart: 'Profile 起点', cleanProfile: '全新 profile', cleanProfileHelp: '在新的隔离浏览器中登录。', copyProfile: '复制现有 profile', copyProfileHelp: '把一个本地 Google Chrome profile 作为 opaque 文件树复制。', copyChromeProfileExperimental: '复制 Google Chrome profile（实验性）', copyChromeProfileExperimentalHelp: '尝试从 Google Chrome 进行 opaque 复制；部分版本或平台可能无法使用。',
    profileImportExperimentalHelp: '实验性功能：仅支持 Google Chrome。Edge、Chromium、Chrome for Testing、Brave、Arc 和其他浏览器均不可导入。Profile 能够成功打开也不代表登录状态一定能够迁移。',
    browserProfiles: '本地 Google Chrome profile', profileScanPrivacy: '这里只显示名称和版本。', scanProfiles: '扫描', scanAgain: '重新扫描', sourceProfile: '来源 profile', sourceBrowser: '浏览器', profileRootPath: 'Profile 根目录', chromeProfileRootPath: 'Google Chrome profile 根目录', profileRootPathPlaceholder: '/用户数据目录的绝对路径…', customProfileRoot: '自定义 profile 根目录', scanProfilesHelp: '需要选择已有 Google Chrome profile 时再扫描。', incompatible: '不兼容',
...
const uiErrorZh: Record<string, string> = {
  ...
  profile_import_browser_unsupported: '实验性 profile 导入仅支持 Google Chrome。',
  profile_import_runtime_unsupported: '实验性 profile 导入只在设置 CloakBrowser 时可用；managed Chrome for Testing 始终使用 clean profile。',
