...
- Category charts use the first seven Tableau 10 colors in published order: blue `#4E79A7`, orange `#F28E2C`, red `#E15759`, teal `#76B7B2`, green `#59A14F`, yellow `#EDC949`, and purple `#AF7AA1`. Keep the fixed family mapping; use Tableau blue for the main series and ColorBrewer Greens[5] for demand intensity. The demand view uses compact squares with no permanent numbers; hover, keyboard focus, or tap reveals counts and their definition.
...
- Capability demand uses **requirement counts**, never the token icon. Each distinct capability counts once per finished job, including failed and canceled jobs; a job can count more than once in a family. Neutral empty squares mean connected with no recorded demand; dashed squares mean no demand or connected route. Green intensity does not encode success, and cells represent individual provider/capability pairs rather than dates. Show every catalog provider and capability, including unused entries; keep headers visible while scrolling. The title follows the selected date range, and counting details belong in help.

Data-color roles follow the [Carbon distinction between categorical, sequential, and alert palettes](https://carbondesignsystem.com/data-visualization/color-palettes/); exact category values come from [D3 schemeTableau10](https://d3js.org/d3-scale-chromatic/categorical), and quantity values from [D3 schemeGreens[5] / ColorBrewer](https://d3js.org/d3-scale-chromatic/sequential).

Matrix axes use local provider marks and one Lucide icon per capability; hover, focus, or tap shows the name. Date-range titles use digits in both languages. Icon provenance is recorded in [SOURCES.md](../../../assets/providers/SOURCES.md).
