<script lang="ts">
  ...
  onMount(() => {
    ...
    const names = [...stones.map(n => `stone-${n}`), ...statuses.map(s => `status-${s.id}`), ...Array.from({ length: 5 }, (_, i) => `heat-${i + 1}`)]
...
<main class="foundations" data-testid="color-and-units">
  <header>
    ...
    <p>{text('Warm graphite. One palette shared by the Dashboard and this atlas.', '暖石墨色。Dashboard 与此 Atlas 共用一套色板。')}</p>
  </header>

  <section class="foundation-panel brand-intro">
    <div class="brand-sample"><TokenIcon size={64} /><span>01 / GRAPHITE</span><strong>#171715</strong></div>
    <div><p class="eyebrow">{text('Primary color', '主色')}</p><h2>{text('Ink on warm paper', '暖纸上的石墨')}</h2><p>{text('Graphite anchors text, primary actions, and quantitative charts. Warm paper gives the interface its background. Color carries a defined meaning wherever it appears.', '石墨色用于文字、主操作和数量图表，暖纸色用于背景。每种颜色都承担固定含义。')}</p><code>--ink / --canvas</code></div>
  </section>

  <section class="foundation-panel">
    <div class="section-heading"><span>01</span><h2>{text('Foundation scale', '基础色阶')}</h2></div>
    <p>{text('0: surface · 50–150: backgrounds · 200: borders · 300–500: chart fills · 600: secondary text · 950: primary text and actions.', '0：表面 · 50–150：背景 · 200：边框 · 300–500：图表填充 · 600：次要文字 · 950：主要文字与操作。')}</p>
  ...
  <section class="foundation-panel">
    <div class="section-heading"><span>02</span><h2>{text('Quantity & categories', '数量与类别')}</h2></div>
    <p>{text('Heatmaps use five fixed steps from light to dark, relative to the largest visible count. Darker means more demand, not better results. Category shades are fixed in the order below; labels and segment separators remain visible.', '热力图相对于当前最大计数，使用固定的五档色阶。越深表示需求越多，不表示结果越好。类别按照下面的固定顺序用色，并保留文字图例和分段边界。')}</p>
    ...
    <div class="category-list">{#each categories as category, i}<span><i style={`background:var(--chart-family-${i})`}></i>{text(category[0]!, category[1]!)}</span>{/each}</div>
  </section>

  <section class="foundation-panel">
    <div class="section-heading"><span>03</span><h2>{text('Semantic colors', '状态色')}</h2></div>
    <p>{text('Reserved for status, never assigned to a provider or capability category. Normal chart series stay graphite. Keep a status label or icon alongside the color.', '只用于状态，不分配给 Provider 或能力类别。普通图表使用石墨色系。状态颜色始终配合文字或图标。')}</p>
  ...
  <section class="foundation-panel">
    <div class="section-heading"><span>04</span><h2>{text('Token unit', 'Token 单位')}</h2></div>
...
<style>
  ...
  .brand-sample { display: flex; min-height: 220px; flex-direction: column; align-items: flex-start; justify-content: space-between; padding: 24px; border-radius: 6px; background: var(--ink); color: var(--surface); }
  ...
  .category-list { display: flex; flex-wrap: wrap; gap: 14px 20px; margin-top: 24px; }
  .category-list span { display: inline-flex; align-items: center; gap: 8px; font-size: 12px; }
  .category-list i, .status-grid i { width: 12px; height: 12px; border-radius: 3px; }
