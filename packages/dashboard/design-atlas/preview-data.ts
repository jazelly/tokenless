...
export function createPreviewSnapshot(language: Language): DashboardSnapshot {
  ...
  return {
    ...
    capabilities: [
      { id: 'conversation.chat', title: language === 'en' ? 'Conversation' : '对话', description: language === 'en' ? 'Ask questions and follow up.' : '提问与追问。', family: 'conversation' },
      { id: 'document.input', title: language === 'en' ? 'Document analysis' : '文档分析', description: language === 'en' ? 'Review and summarize documents.' : '评审与总结文档。', family: 'input' },
      { id: 'search.web', title: language === 'en' ? 'Web research' : '网络研究', description: language === 'en' ? 'Find and compare sources.' : '查找与比较来源。', family: 'retrieval_reasoning' },
...
export function previewAnalytics(snapshot: DashboardSnapshot, profileId: string, range: DashboardAnalyticsRange): DashboardAnalytics {
  ...
  return {
    ...
    capabilities: [{ capabilityId: 'conversation.chat', family: 'conversation', succeededJobs: count, failedJobs: 0, canceledJobs: 0, finishedJobs: count, successRate: 1, providersUsed: providers.length }],
    capabilityFamilies: [{ family: 'conversation', succeededJobs: count, failedJobs: 0, canceledJobs: 0, finishedJobs: count }],
    capabilityMatrix: providers.map(provider => ({ provider, capabilityId: 'conversation.chat', family: 'conversation', succeededJobs: count / providers.length, failedJobs: 0, canceledJobs: 0, finishedJobs: count / providers.length })),
