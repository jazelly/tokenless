...
const enMessages = {
    ...
    capabilityUsageMatrix: 'Capability demand by provider', capabilityUsageMatrixHelp: 'Each square is one provider and capability category. Darker green means more recorded demand, not a higher success rate. Hover, focus, or tap for counts. Unit: capability requirements, not API calls or tokens. Each distinct capability requested by a finished job counts once. A job requesting file upload and document input adds 2 to Input. Succeeded, failed, and canceled jobs all count; running jobs do not. Counts follow the selected range and profile and do not prove successful execution.', routedRequirements: 'capability requirements', supportedUnused: 'Connected capability; no recorded demand in this range', unsupportedCapability: 'No recorded demand or connected route; this does not describe the provider website',
    matrixReadingHint: 'One square per provider and capability category · hover or tap for demand counts',
    matrixCountMeaning: 'Each distinct capability requested by a finished job counts once, including failed and canceled jobs. Upload + document input in one job adds 2 to Input. Not API calls or tokens.',
    matrixTooltipMeaning: 'Each requested capability counts once per finished job. These are requirements, not API calls or tokens.',
    matrixLess: 'Less', matrixMore: 'More',
    matrixFamily_conversation: 'Chat', matrixFamily_input: 'Input', matrixFamily_retrieval_reasoning: 'Search', matrixFamily_media_generation: 'Media', matrixFamily_artifact_generation: 'Docs', matrixFamily_workspace_knowledge: 'Projects', matrixFamily_evidence_lifecycle: 'Controls',
...
const zhMessages: Record<keyof typeof enMessages, string> = {
    ...
    tokenUnitHelp: 'tokens · 模型处理文本的计量单位。圆框 T 是 Tokenless API 的单位图标，不是货币符号。一个 token 不一定对应一个字或一个词。', capabilityCountUnit: '单位：能力需求次数 · 包含失败和取消的任务', jobsCountUnit: '单位：任务数', estimatedTokenUnit: '单位：tokens · 按可见输出估算', matrixZeroLegend: '无需求记录', matrixAbsentLegend: '无需求记录且未接入', matrixCellOutcomes: '成功 {succeeded} · 失败 {failed} · 取消 {canceled}（能力需求次数）',
    ...
    capabilityUsageMatrix: '各 Provider 的能力需求次数', capabilityUsageMatrixHelp: '每个方格对应一个 Provider 和一类能力。绿色越深表示需求越多，不表示成功率越高。悬停、聚焦或点按可查看次数。单位是能力需求次数，不是 API 调用次数，也不是 tokens。已结束任务中，每种不同的能力需求各计 1 次。例如一个任务同时要求上传文件和文档输入，就给「输入」计 2 次。成功、失败、取消都计入，运行中不计入。统计跟随所选时间和 Profile，不代表能力已执行成功。', routedRequirements: '次能力需求', supportedUnused: '已接入该类能力，但所选时间内无需求记录', unsupportedCapability: '无需求记录且未接入该类能力；不表示 Provider 官网不支持',
    matrixReadingHint: '每格代表一个 Provider 的一类能力 · 悬停或点按查看需求次数',
    matrixCountMeaning: '已结束任务中，每种不同的能力需求各计 1 次，包含失败和取消。一个任务同时要求上传文件和文档输入，就给「输入」计 2 次。不是 API 调用次数，也不是 tokens。',
    matrixTooltipMeaning: '每个已结束任务中，每种能力需求各计一次。这是需求次数，不是 API 调用次数或 tokens。',
    matrixLess: '少', matrixMore: '多',
    matrixFamily_conversation: '对话', matrixFamily_input: '输入', matrixFamily_retrieval_reasoning: '检索', matrixFamily_media_generation: '媒体', matrixFamily_artifact_generation: '文档', matrixFamily_workspace_knowledge: '项目', matrixFamily_evidence_lifecycle: '控制',
