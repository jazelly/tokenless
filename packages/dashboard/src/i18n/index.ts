...
const enMessages = {
    ...
    analyticsTitle: 'Usage analytics', analyticsLede: 'See which providers Tokenless API uses, which capabilities workloads require, and how outcomes change over time.', analyticsLoading: 'Loading usage analytics…', analyticsUnavailable: 'Usage analytics could not be loaded.', retryAnalytics: 'Try again',
    ...
    providerPanorama: 'Provider Panorama', providerPanoramaHelp: 'Finished jobs are attributed to the final provider.', byJobs: 'Jobs', byTokens: 'Measured tokens', bySuccess: 'Success', jobShare: 'Job share', lastUsed: 'Last used', capabilitiesUsed: 'capabilities used',
    ...
    executionMix: 'Execution mode mix', unknownMode: 'Unknown mode', insightMostUsed: '{provider} handled {share} of finished jobs.', insightCapability: '{capability} was the most requested capability.', insightConcentration: 'The top two providers handled {share} of finished jobs.', insightUnused: '{count} catalog capabilities were not requested in this range.', insightFailure: '{provider} × {family} recorded the most failed requirements ({count}).', insights: 'Insights',
...
const zhMessages: Record<keyof typeof enMessages, string> = {
    ...
    analyticsTitle: '使用分析', analyticsLede: '查看 Tokenless API 主要使用哪些 provider、工作负载需要哪些 capability，以及结果如何随时间变化。', analyticsLoading: '正在读取使用分析…', analyticsUnavailable: '无法读取使用分析。', retryAnalytics: '重试',
    ...
    providerPanorama: 'Provider 全景', providerPanoramaHelp: '已结束任务归属于最终完成的 provider。', byJobs: '任务', byTokens: '已测量 token', bySuccess: '成功率', jobShare: '任务占比', lastUsed: '最近使用', capabilitiesUsed: '个 capability 已使用',
    ...
    executionMix: '执行模式构成', unknownMode: '未知模式', insightMostUsed: '{provider} 承担了 {share} 的已结束任务。', insightCapability: '{capability} 是请求最多的 capability。', insightConcentration: '前两个 provider 承担了 {share} 的已结束任务。', insightUnused: '所选范围内有 {count} 个 catalog capability 未被请求。', insightFailure: '{provider} × {family} 记录了最多失败 requirement（{count} 次）。', insights: '洞察',
    ...
    chromePromptApiEngine: 'Chrome Prompt API · Gemini Nano', routerEngineHelp: 'V1 只有一个本地 Engine；这个边界允许将来替换成其他本地或远程 Engine。', routerDisabledError: '请先启用 Experimental Router，再运行实验。',
    rendererBrowser: 'Dashboard 所在浏览器', browserVersion: 'Renderer 浏览器版本', routerRequirement: 'Engine 要求', routerBrowserRequirement: 'Google Chrome 148+', routerBrowserModeUnsupported: 'Dashboard 所在浏览器模式当前不能使用这个 Engine。', routerBrowserUnsupported: 'Dashboard 当前不是运行在 Google Chrome 中；请在 Google Chrome 中打开 Dashboard 后再使用这个 Engine。', routerBrowserVersionUnsupported: 'Chrome Prompt API 要求 Google Chrome 148 或更高版本。检测到的 renderer 版本：',
    ...
    routerTest: '语义路由实验', routerTestHelp: 'Nano 运行前，Tokenless 会排除没有 verified Harness 附件 route 或已知容量不足的 provider。Nano 会收到剩余 provider 的规则、所选 model、当前 profile plan 和已知额度。', taskPrompt: '任务 prompt', taskPromptPlaceholder: '用自信的语气起草一份产品发布公告。', runSemanticRouter: '运行语义路由', routerRunning: '路由中…', routerResult: '结构化结果', startHarnessRun: '启动 Harness run', harnessStarting: '正在启动 Harness run…', harnessRun: 'Harness run', harnessProfileRequired: '请先选择一个 managed profile，再启动 Harness run。', routerSemanticManifest: 'Terminal-Bench 语义 manifest', routerSemanticManifestHelp: '通过本地 Chrome Prompt API 分类全部 89 个 pinned instruction，并写入可通过 validator 的 external manifest。', routerSemanticManifestTarget: '请使用 tokenless dashboard --semantic-manifest-output <absolute-path> 打开此 Dashboard。这个 target 只能使用一次，并在 30 分钟后过期。', routerSemanticManifestRun: '分类 89 个 task', routerSemanticManifestRunning: '分类中…', routerSemanticManifestSaved: 'Semantic manifest 已保存', routerManifestTargetRequired: '请先使用明确的 semantic manifest output target 打开此 Dashboard。', routerManifestTaskTypeInvalid: 'Chrome Prompt API 返回了不符合边界的 task type。', downloadProgress: '模型下载', downloadZeroHelp: '0% 表示 Chrome 已接受下载请求，但尚未报告进一步的模型进度。请查看下方的 Model Status 和 Event Logs。',
