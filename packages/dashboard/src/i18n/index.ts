...
const enMessages = {
    ...
    createProfile: 'Create profile', slug: 'Profile slug', role: 'Display name or purpose', visibility: 'Visibility', enabledProviders: 'Enabled providers',
    ...
    appearance: 'Appearance', profile: 'Profile', connection: 'Connection', advanced: 'Advanced', edit: 'Edit', more: 'More', close: 'Close', quickSettings: 'Quick settings', quickSettingsHelp: 'Change everyday preferences without leaving this page.',
    ...
    chromePromptApiEngine: 'Chrome Prompt API · Gemini Nano', sparkX25MlxEngine: 'Spark X2.5-4B · local MLX', routerEngineHelp: 'V1 provides two local engines (Chrome Prompt API · Gemini Nano and the fixed Spark X2.5-4B MLX server) plus Jev, which runs remotely through TypeSafe.', routerDisabledError: 'Enable Experimental Router before running a test.',
    ...
    jevEngine: 'Jev · TypeSafe System One', jevSetup: 'Configure Jev (TypeSafe)', jevSetupIntro: 'Jev runs remotely through TypeSafe’s hosted API. It returns typed judgments (Choice, Noul, Score), not generated text, so titles are derived locally instead of asked from the model.', jevSetupNote: 'The Tokenless server needs TYPESAFE_API_KEY set in its own process environment before this engine can answer requests. It is not read from this repository’s .env file at runtime.', jevServerRequirement: 'TYPESAFE_API_KEY on the Tokenless server', routerLatency: 'Latency',
    jevPlayground: 'Jev raw playground', jevPlaygroundHelp: 'Send arbitrary state and typed questions straight to Jev’s systemOne endpoint and inspect the structured answer, token usage, and latency.', jevPlaygroundState: 'State (JSON or plain text)', jevPlaygroundStatePlaceholder: '{"document": "I was charged twice. Please fix this ASAP."}', jevPlaygroundQuestions: 'Questions (JSON)', jevPlaygroundRun: 'Send to Jev', jevPlaygroundRunning: 'Sending…', jevPlaygroundResult: 'Response', jevPlaygroundInvalidJson: 'Questions must be valid JSON describing one or more noul/choice/score questions.', jevPlaygroundStateRequired: 'Enter state for Jev to evaluate.', jevApiKey: 'TypeSafe API key', jevApiKeyConfigured: 'A key is already configured on the server; leave this blank to keep it.', jevApiKeyNotConfigured: 'No key is configured yet. Jev falls back to the server’s TYPESAFE_API_KEY env var if this stays blank.', jevApiKeyPlaceholderConfigured: 'Leave blank to keep the saved key', jevApiKeyPlaceholderEmpty: 'Paste your TypeSafe API key', jevApiKeyClear: 'Remove the saved key on save', jevPlaygroundHistory: 'Recent Jev calls', jevPlaygroundHistoryEmpty: 'No Jev calls recorded yet in this session.', jevPlaygroundHistoryError: 'Failed',
} as const
...
const zhMessages: Record<keyof typeof enMessages, string> = {
    ...
    createProfile: '创建 profile', slug: 'Profile slug', role: '显示名称或用途', visibility: '可见性', enabledProviders: '启用的 provider',
    ...
    appearance: '外观', profile: 'Profile', connection: '连接', advanced: '高级', edit: '编辑', more: '更多', close: '关闭', quickSettings: '快捷设置', quickSettingsHelp: '无需离开当前页面，即可更改常用偏好。',
    ...
    chromePromptApiEngine: 'Chrome Prompt API · Gemini Nano', sparkX25MlxEngine: 'Spark X2.5-4B · 本地 MLX', routerEngineHelp: 'V1 提供两个本地 Engine（Chrome Prompt API · Gemini Nano，以及固定的 Spark X2.5-4B MLX server），再加上通过 TypeSafe 远程运行的 Jev。', routerDisabledError: '请先启用 Experimental Router，再运行实验。',
    ...
    jevEngine: 'Jev · TypeSafe System One', jevSetup: '配置 Jev（TypeSafe）', jevSetupIntro: 'Jev 通过 TypeSafe 的托管 API 在远程运行。它返回的是类型化判断（Choice、Noul、Score），不是生成文本，所以标题是本地拼接出来的，不会去问模型。', jevSetupNote: 'Tokenless server 自己的进程环境里需要先配置好 TYPESAFE_API_KEY，这个 Engine 才能响应请求；运行时不会读取这个仓库的 .env 文件。', jevServerRequirement: 'Tokenless server 上的 TYPESAFE_API_KEY', routerLatency: '延迟',
    jevPlayground: 'Jev 原始 Playground', jevPlaygroundHelp: '直接把任意 state 和类型化 questions 发给 Jev 的 systemOne 接口，查看结构化返回、token 用量和延迟。', jevPlaygroundState: 'State（JSON 或纯文本）', jevPlaygroundStatePlaceholder: '{"document": "被重复扣款了，请尽快处理。"}', jevPlaygroundQuestions: 'Questions（JSON）', jevPlaygroundRun: '发送给 Jev', jevPlaygroundRunning: '发送中…', jevPlaygroundResult: '返回结果', jevPlaygroundInvalidJson: 'Questions 必须是合法 JSON，描述一个或多个 noul/choice/score 问题。', jevPlaygroundStateRequired: '请输入要让 Jev 评估的 state。', jevApiKey: 'TypeSafe API Key', jevApiKeyConfigured: 'Server 上已经配置了一个 Key；留空表示不修改。', jevApiKeyNotConfigured: '还没有配置 Key。留空的话 Jev 会退回用 server 自己的 TYPESAFE_API_KEY 环境变量。', jevApiKeyPlaceholderConfigured: '留空以保留已保存的 Key', jevApiKeyPlaceholderEmpty: '粘贴你的 TypeSafe API Key', jevApiKeyClear: '保存时清除已保存的 Key', jevPlaygroundHistory: '最近的 Jev 调用', jevPlaygroundHistoryEmpty: '本次 session 里还没有 Jev 调用记录。', jevPlaygroundHistoryError: '失败',
}
