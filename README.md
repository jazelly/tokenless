...
Every managed profile is bound to the browser runtime that created it. Tokenless does not silently open a managed profile with a different runtime family. A runtime-family change normally creates a clean profile. Experimental import accepts only one version-compatible local Google Chrome profile for opaque filesystem copy into a new managed profile; Tokenless does not inspect or expose individual cookies, tokens, browser storage, or authentication values. Import may fail by browser version or platform, and a successful open does not guarantee sign-in-state transfer. The managed browser then preserves any session it can use across jobs.
