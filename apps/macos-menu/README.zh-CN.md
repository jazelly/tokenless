...
应用启动时会立即执行内置的 `tokenless menubar status --json`，确保 Tokenless API daemon 已就绪。因此开启“登录时启动 Tokenless”就会启动完整的本地 Tokenless 控制面，包括由 daemon 提供的 Dashboard；provider 浏览器仍然按需启动。
